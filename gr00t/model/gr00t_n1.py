...
from .action_head.feedback_action import FeedbackAction
...
class GR00T_N1_5(PreTrainedModel):
    ...
    # def get_action(  # 2. Skip VLM steps
    #     self,
    #     inputs: dict,
    #     time_step: int,
    # ) -> BatchFeature:
    ...
    def get_action(  # 4. Update Action, test
        self,
        inputs: dict,
        time_step: int,
    ) -> BatchFeature:
        backbone_inputs, action_inputs = self.prepare_input(inputs)
        print(f"[MODEL] im at STEP {time_step}, Run Model")
        print("320 320 320")
        backbone_outputs = self.backbone(backbone_inputs)
        action_head_outputs = self.action_head.get_action(backbone_outputs, action_inputs)
        self._cached_action_head_outputs = action_head_outputs  # save 16 raw actions

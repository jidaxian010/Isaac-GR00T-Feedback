    ...
    def _materialize_meta_tensors(self):
        ...
        for module in self.modules():
            if isinstance(module, nn.Conv2d):
                ...
                print(
                    f"[DEBUG] Materialized Conv2d layer: weight range [{module.weight.min().item():.6f}, {module.weight.max().item():.6f}]"
                )
                ...
                if module.bias is not None:
                    module.bias.data = module.bias.data.clone()
                print(
                    f"[DEBUG] Materialized Linear layer: weight range [{module.weight.min().item():.6f}, {module.weight.max().item():.6f}]"
                )

...
class PolicyEvaluator:
    ...
    def compare_results_libero(self):
        ...
        for time_step in range(self.num_episodes):  # 132 is the number of steps in the dataset for the first task
            ...
            if time_step % action_horizon == 0:
                ...
                action_chunk = self.policy.get_action(
                    step_data, time_step, select_model_fast=False, print_timing=True
                )["action.all_actions"]
                print("ACTION CHUNK SHAPE", action_chunk.shape)

                if action_chunk.shape[0] != action_horizon:
                    raise ValueError(
                        f"Action chunk shape {action_chunk.shape[0]} does not match expected action horizon {action_horizon}"
                    )
        ...
        print(
            f"Average DiT (action head) time: {np.mean(action_head_times):.4f} ± {np.std(action_head_times):.4f} seconds"
        )
        ...
        for i in range(7):
            ...
            axs[i].plot(
                predicted_action_chunk_array[:, i], label="Predicted Action Chunk", color="black", linestyle="--"
            )
            axs[i].set_title(f"{joint_names[i]}")
            axs[i].set_xlabel("Time Step")
            axs[i].set_ylabel("Joint Value")
    ...
    def compare_results_libero_fast(self):
        ...
        for time_step in range(self.num_episodes):
            ...
            if not np.array_equal(step_data["video.agentview_rgb"], step_data["obs.agentview_rgb"]):
                raise ValueError(
                    f"shape of video {step_data['video.agentview_rgb'].shape} and shape of obs {step_data['obs.agentview_rgb'].shape}"
                )
            ...
            action_chunk = self.policy.get_action(step_data, time_step, select_model_fast=True, print_timing=True)[
                "action.all_actions"
            ]
...
def main():
    ...
    evaluator = PolicyEvaluator(
        ...
        dataset_path=os.path.join(
            os.path.dirname(os.path.dirname(gr00t.__file__)), "demo_data/libero_object_data"
        ),
